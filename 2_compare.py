...
print 'use image "Lenna.png" and auto threshold to compare with opencv function'

### read image and convert to grayscale
img = cv2.imread('input_image.png')			    # read image
grayimg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # convert image to grayscal

### Canny edge ditector with OpenCV function
opencv_canny = cv2.Canny(grayimg,100,200)
...
highholds = 0.2*m 		# set upper bound
lowholds = 0.1*m  		# set lower bound

"""
highholds = 0.3*m 		# set upper bound
lowholds = 0.1*m 		 # set lower bound
"""
...
for i in range(0, x):							 # count image pixel from 0 to x
    for j in range(0, y):						 # count image pixel from 0 to y
        if magnitude[i,j]>=highholds: 			 # if pixel magnitude value > upper bound
            M_above_high[i,j] = magnitude[i,j]	 # store to M_above_high
        if magnitude[i,j]>=lowholds:			 # if pixel magnitude value > lower bound
